...
*!-------------------------------------------------------------------------------------+
*! flexmat version 1.8: Jan 07, 2023: added appendmat for appending flexmat files      |
...
* Change log  :
* flexmat version 1.7: Oct 20, 2022: empty notes and title replaced with empty unicode|
* version 1.6 : Mar 4, 2022: addmat issue fixed                               |
* version 1.6 : Mar 4, 2022: addmat issue fixed. When Stata matrix was added with addmat, it would leave the first cell blank. that has been fixed. 
* version 1.5 : Nov 2021 : dropcol and droprow now accepts range row(1-4), col(2-4) or list row(1,3,5)
* version 1.4 : March 14, 2021 : title, notes, and fmtmat changed
* verson  1.3 : Jan 26, 2021 : getlocinfo moved to option from sub-command
* verson  1.2 : Jan 22, 2021 : fixed issue in insertrow
...
program define flexmat
	syntax anything, 	///
		...
		mode(str) 		///
		change(str)		/// to change existing text with new, i.e. change("#", "x")
		remove(str)		/// to remove text, i.e. remove("."), or remove(prefix) will remove c. from c.trunk, remove(suffix) will remove .trunk from c.trunk
		info			/// to display flexmat information, e.g active file name, active location, etc.
		]
	...
	if ("`anything'" == "clear") loc anything anything reset
	...
	if (strmatch("`anything'", "*merge*")) {
	...
	else if (strmatch("`anything'", "*appendmat*")) {
	    append_matrices, flexmatfile(`flexmat_stored_matrix_file_name')  address_of_the_appendable_item(`matname')
		exit
	}
		...
		if ("`location'" == "") loc location = 1
 		...
 		if (`"`anything'"' == "addparts") {
			...
			// if notes are empty, then fill it with unicode empty character https://www.compart.com/en/unicode/U+2800
			if ("`notes'" == "") loc notes ⠀
			if ("`title'" == "") loc title ⠀
	...
	if (!inlist(`"`anything'"', "addcell", "addtext", "addcol", "addrow", "reset", ///
		...
		"insertmat", "dropcol", "droprow", "insertcell", "addfmtmat")) & ///
		(!inlist(`"`anything'"', "change", "getlocinfo")) {
			
			dis as error "The sub-command `anything' is unrecognized"
			exit

	}
	if !inlist(`"`anything'"', "addmat", "addcol", "addrow", "insertcol", "insertrow", "insertmat", "insertcell") {
	...
	*--------------------------------------------
	...
	else  	if ("`anything'" == "insertcell") insertcell, data("`data'") 	row(`row') col(`col') dec(`dec') parse(`parse') file(`file') loc(`location') `left'`right'`below'`above'
	...
	else 	if ("`anything'" == "change")	change_text, `change' col(`col') row(`row') file(`file') loc(`location')
...
// Program change_text replaces old str with new str in the flexmat table
program define change_text
	
	syntax, change(str) [file(str) Row(int 1) Col(int 1) loc(str)]
	

	gettoken  oldtext change : change, parse(",") 
	gettoken  comma change   : change, parse(",") 
	loc newtext = subinstr("`change'", " ", "", 1)
	
	mata: change_text("`file'", `loc', `row', `col', "`oldtext'", "`newtext'")
...
prog append_matrices


	syntax, flexmatfile(str)  address_of_the_appendable_item(str) [swap(str)]
	
	loc stata_version = `c(version)'
	mata: getlocinfo("`flexmatfile'")
	
	local loc = $flexmat_current_loc + 1
	
	// confirm that file exists
	
	loc file_does_not_exist 1
	
	cap confirm file `"_asdoc/`address_of_the_appendable_item'.flexmat"'
	if (_rc == 0) {
	    loc address_of_the_appendable_item "_asdoc/`address_of_the_appendable_item'.flexmat"
		loc file_does_not_exist 0
	}
	
	// if file does not exist, continue search
	
	if (`file_does_not_exist') {
		cap noi confirm file `"_asdoc/`address_of_the_appendable_item'"'
		if (_rc == 0) {
			loc address_of_the_appendable_item `"_asdoc/`address_of_the_appendable_item'"'
			loc file_does_not_exist 0
		}
	}
	
	if (`file_does_not_exist') {
		cap noi confirm file `"`address_of_the_appendable_item'"'
		if (_rc == 0) {
		    loc address_of_the_appendable_item `"`address_of_the_appendable_item'"'
			loc file_does_not_exist 0
		}
	}
			
	if strmatch("`address_of_the_appendable_item'", "*.flexmat") {
		
		
		mata: append_flexmat_item_to_colection("`address_of_the_appendable_item'", "`flexmatfile'",`loc', "`swap'")
	}
	
	else {
		
		//mata : append_matrices("`flexmatfile'", "`address_of_the_appendable_item'", `stata_version')
		mata: append_full_matrix("`address_of_the_appendable_item'", "`flexmatfile'", `loc', "`swap'")
...
prog insertcell
	...
	mata: insertcell("`file'", `loc', "`dec'", "`data'", "`parse'", `row', `col')
	...
	if _rc == 601 { // if Stata matrix
		...
		if _rc != 111 {
			...
			if ("`nonames'" == "" ){
				...
				if ("`nocolnames'" == "") loc  adjusted_row "+1"
			}
			...
			if ("`norownames'" == "") & ("`nonames'"=="") {
				...
				forv i = 1  / `ROW' {
					...
					loc mat_row = `i' + `row' `adjusted_row' - 1					
			...
			loc row = `row' `adjusted_row'
			loc col = `col' `adjusted_col'
...
cap prog drop flexmat_info
program define flexmat_info 
	dis as text "active flexmat file : " as res "$active_flexmat_file"
	dis as text "Next write location : " as res "$flexmat_current_loc"
