{smcl}
{right:version:  1.4.0}
{cmd:help flexmat} {right:Jan 07, 2023}
{hline}
...
This program is specifically useful when numeric and string results need to be
...
{synopt :{help flexmat##appendmat:appendmat}}Appends a flemxat matrix on disk to the one in memory (active flexmat file) {help flexmat##matname:matname()} {p_end}
{synopt :{help flexmat##change:change}}Replaces old text with new text {p_end}
{synopt :{help flexmat##reset:reset | clear}}deletes existing matrices and clears memory {p_end}
{synopt :{help flexmat##notes:addparts}}Add table notes  and | or title{p_end}
...
{synopt :{opth parse:(flexmat##parse:comma, space, pipe)}}specify the parsing character used in the option data{p_end}
...
The data items in the option data should be separated by a comma or any other delimters.
...
Let us continue writing to the same matrix that we started in the above example.
...
required options : {opt data(data1, data2, data3, ...)}, {opt row(int)}, and {opt col(int)}. 
The data items in the option data should be separatedby a comma or any other delimiters.
...
Let us continue writing to the same matrix that we started previously. 
...
add it to our previously created table. To avoid the matrix row and column names, 
...
Therefore{opt insertrow} will always increase the row count of the matrix by one. 
...
In other aspects, it is similar to the {opt addrow} option. {break}
...
{stata "flexmat insertcol, data(Median, 4, 5, 7, 9,0.370,0.370) row(1) col(6)": flexmat insertcol, data(Median, 4, 5, 7, 9, 0.370, 0.598) row(1) col(6)} {break}
...
{p 4 4 2}With the insert of one more olumn, our table now looks like:
	...
	----+-------------------------------------------------------------------------------
	  ...
	  7 | 0.370                  0.370       0.370       0.370       0.370     0.598 
	-----------------------------------------------------------------------------------
{marker droprow}
...
{marker merge}
 ...
 of contents of the two matrices in their first column. In other words, the two
 matrices are merged on their first columns. Values that are not merged, are
 ...
 at location 1 and 2, we shall use {opt matloc(1,2)}.
  {marker keep}
  
 {p 8 8 1}{opt 11.2 keep(#,#)}: Further, we can use option {opt keep(#,#)} to keep either
 the first matrix, the second matrix, or both
 in the flexmat system. If option {opt keep(#,#)} is not used, flexmat will delete
 these matrices and will keep just the newly merged matrix. Option {opt keep(#,#)} 
...
default location of 1. Let us first create another matrix at location 2, and
...
loc = 2, that show the location information of the two matrices.
	...
	----+-------------------------------------------------------------------------------
	  ...
	  7 |0.349                 0.349       0.349       0.349       0.349     0.598 
	------------------------------------------------------------------------------------
	...
	{p 8 8 2}The two matrices have three values in common in their first rows:
	...
	{marker appendmat}
{p 4 4 1}{cmd: 12. appendmat} : It appends a flexmat file to the current asdocx
file (that is is available in the global macro {hi:{it:$active_flexmat_file}}). You 
must use the option {opt matname()} when using {opt appendmat()}. This sub-command
is similar to {opt addmat}, with two differences: {break}

{p 8 8 2}1. You do not have to spcecify the {opt location()} option as {opt appendmat}
automatically identifies the next available location and appends the flexmat file to it.

{p 8 8 2}2. Files written with flexmat are stored as associative array, therefore, 
they need to be converted back to matrices before appending them to other files. 
The sub-command {opt appendmat} takes care of this.

{p 4 4 1}Here is a typical use of the sub-command {opt appendmat}. 

{p 8 8 2}*{cmd: Specify the flexmat filename using asdocx} {break}
asdocx setfile, save(First_Test){break}

{p 8 8 2}*{cmd: Add contents to the file First_Test} {break}
flexmat addrow, data(Year, Month, Day) r(1){break}
flexmat addrow, data(2001, March, 21) r(2){break}
flexmat addparts, title(Table: Test Results for Sample A) notes(Sample A was tested for contaminants X, Y, and Z.)

		
{p 8 8 2}*{cmd: Create another flexmat file and add contents} {break}
asdocx setfile, save(Second_Test){break}
flexmat addrow, data(Make, Model, mpg) r(1){break}
flexmat addrow, data(Ford, Explorer, 15) r(2){break}
flexmat addparts, title(Table: Test Results for Sample B) notes(Sample B was tested for contaminants A, B, and C)


{p 8 8 2}*{cmd: Create asdocx file and append the earlier two flexmat files to it} {break}
asdocx, save(REPORT) text(MY REPORT) title replace {break}
flexmat appendmat, matname(First_Test){break}
flexmat appendmat, matname(Second_Test){break}

{p 8 8 2}*{cmd: Export the asdocx file to MS Word} {break}
asdocx export{break}

{p 4 4 2}*{cmd: Notes on file names}{break}
The {opt setfile} option of 'asdocx' causes 'flexmat' to store its files inside the 
{opt _asdoc} folder, which is created inside the user's current directory. 
This folder is hidden by default to prevent visual clutter and accidental deletion. 
In the examples given, the files {opt First_Test} and {opt Second_Test} are stored as 
{opt First_Test.flexmat} and {opt Second_Test.flexmat}, respectively, in the hidden 
directory. Users do not need to worry about specifying the folder path or the 
extension, as flexmat automatically retrieves files from the hidden
 directory and adds the {opt .flexmat} extension. 


{marker change}
{p 4 4 2} {cmd: 13.} {opt change(old_string, new_string)} : replaces old string with 
new string at the current location. In the following code, all instances
of the string {hi:#} will be replaced with the string {hi: x }.

	{p 8 8 2}{stata `"flexmat change("#", " x ")"': flexmat change("#", " x ")} {break}

 {marker showmat}
{p 4 4 2} {cmd: 14. showmat} : displays the contents of the active flexmat file.
The sub-command does not need any option. However, we can use {opt locinfo} to 
report location information with each table.

{p 8 8 2}{stata "flexmat showmat": flexmat showmat} {break}
{stata "flexmat showmat, locinfo": flexmat showmat, locinfo} {break}
...
{p 8 8 2}The concept of locations in flexmat is to create separate entities or
...
{p 4 4 2}{cmd: 8. reset | clear} : {break}Option {opt reset} or {opt clear} deletes any stored matrices. If the reset is used 
without any option, it will delete the default matrices, i.e stored_results.flexmat file in the current directory.
...
{marker notes}
{p 4 4 2}{cmd: 9. addparts}: {break}Sub-command {opt addparts} with option 
{opt notes(Table notes text)} adds table notes to the current table. 
Table notes apear at the bottom of the table when the table is exported with asdocx.
 When sub-command {opt addparts} is used with option {opt title(Table title text)}, it sets the
table title. {break}

{p 4 8 2} {hi:Example 8 - Set table notes} {break}
{stata "flexmat addparts, notes(Source: The data is collected by ... )" : flexmat addparts, notes(Source: The data is collected by ...)}{break}

{p 4 8 2} {hi:Example 9 - Set table title} {break}
{stata "flexmat addparts, title(Table 5: Descriptive Statistics)" : flexmat addparts, title(Table 5: Descriptive Statistics)}{break}
...
{title:Acknowledgement}
  {p 4 4 2} I am grateful to Kevin Blaine for his ongoing engagement with Flexmat. 
 His inquiries and recommendations have enhanced the functionality of several
  sub-commands. The examples provided in the {help flexmat##appendmat:appendmat} section were 
  taken from his post on the 
  {browse "https://fintechprofessor.com/asdocx/forums/topic/creating-multiple-custom-tables-at-once-using-flexmat-then-output-to-one-doc/#post-17430":asdocx forum}.
